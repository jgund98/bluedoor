// Throwaway capture rig for the /97 lander on the epic dev server.
// Usage: node scripts/shot97.mjs <outDir> <width> <port> [route]
...
const PORT = process.argv[4] ?? "3700";
const ROUTE = process.argv[5] ?? "/97";
...
console.log(JSON.stringify({ route: ROUTE, width: WIDTH, loadMs, pageHeight: total, frames, errors }, null, 1));
await browser.close();
